...
- 对话自动检索（AI 协作自动反哺）：规则门控触发（寒暄/确认不检索）、会话级缓存复用、
  token 预算与召回条数上限、置信度门槛过滤、强制 [知识N] 溯源、低置信度/时效性问题
  自动指示 AI 联网核实（web_search/web_fetch），无法核实时明确标注“未验证”。
  开关与参数在知识库页「向量/维护」区（对话自动检索）配置。
