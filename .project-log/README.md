...
项目日志采用“地图 → 路由 → 详情”的渐进式披露，避免每次任务把完整历史塞进上下文：

1. **项目地图**：先读项目根目录的 README、manifest、主要源码目录和构建/测试入口，了解项目用途、语言、框架和边界。
2. **当前状态**：只读 `status.md` 顶部摘要和“最近记录编号”；需要历史时再查 `index.md` 的模块、路径和关键词索引。
3. **详情**：定位后只读匹配的 `changelog.md` 或 `learnings.md` 条目。默认不全文读取这两份文件。
4. **扩大范围**：仅在定位不足或发现冲突时按“当前条目 → 当前模块 → 相关时间段 → 全量日志”逐级扩大，并说明原因。
5. **蒸馏例外**：`project-log-distill` 默认只读取未标记“已蒸馏”的条目，全文复盘必须由用户明确要求。

技能执行结束时应报告实际读取的日志文件和条目范围；未读取的历史不能当作已核验事实。
