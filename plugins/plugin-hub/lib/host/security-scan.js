...
/** Skip dependencies, VCS data and generated coverage; shipped build output is scanned. */
const SKIP_DIRS = new Set(['node_modules', '.git', 'coverage', '.next', '.nuxt']);
...
function scanText(text, target, findings) {
...
export function scanPlugin(dir, pkg) {
    const findings = [];
    ...
    for (const file of files.slice(0, MAX_SCAN_FILES)) {
        if (scannedBytes + file.size > MAX_SCAN_BYTES) {
            findings.push({ severity: 'warning', rule: 'scan-budget-exceeded', target: file.path, detail: '插件代码超过安全扫描总量上限，无法完成审批' });
            break;
        }
        let text = '';
        try {
            text = readFileSync(join(dir, file.path), 'utf8');
        }
        catch {
            continue;
        }
        ...
        scanText(text, file.path, findings);
    }
...
export function isBlocked(report) {
