...
export function storeDataDir(home = resolveDshHome()) {
...
export class StoreDB {
    ...
    constructor(home = resolveDshHome()) {
        ...
        try {
            this.db.exec('ALTER TABLE plugins ADD COLUMN trust_level TEXT');
        }
    ...
    `).run(row.packageName, row.repoFullName, row.repoUrl, row.name, row.description, row.shortDescription, row.developer, row.iconUrl, row.categories, row.version, row.stars, row.downloads, row.forks, row.topics, row.license, row.homepage, row.defaultBranch, row.readme, row.usage, row.screenshots, row.packageJson, row.updatedAt, row.syncedAt);
    }
    getPlugin(packageName) {
        const row = this.db.prepare('SELECT * FROM plugins WHERE package_name = ?').get(packageName);
        return row === undefined ? undefined : mapPluginRow(row);
    }
    getPluginByRepo(repoFullName) {
        const row = this.db.prepare('SELECT * FROM plugins WHERE repo_full_name = ?').get(repoFullName);
        return row === undefined ? undefined : mapPluginRow(row);
    }
    countPlugins() {
        const row = this.db.prepare('SELECT COUNT(*) AS n FROM plugins').get();
        return Number(row.n);
    }
    /** List every mirrored plugin row (used by the dependency-graph scan). */
    allPlugins() {
        const rows = this.db.prepare('SELECT * FROM plugins ORDER BY name').all();
        return rows.map(mapPluginRow);
    }
    // ---- versions -----------------------------------------------------------
    replaceVersions(packageName, versions) {
        ...
        for (const v of versions)
            stmt.run(packageName, v.version, v.publishedAt, v.changelog, v.tagName);
    }
    listVersions(packageName) {
        const rows = this.db.prepare('SELECT version, published_at AS publishedAt, changelog, tag_name AS tagName FROM versions WHERE package_name = ? ORDER BY published_at DESC').all(packageName);
        return rows;
    }
    // ---- dependencies --------------------------------------------------------
    replaceDependencies(packageName, edges) {
        ...
        for (const edge of edges)
            stmt.run(packageName, edge.name, edge.kind, edge.version);
    }
    dependenciesOf(packageName) {
        const rows = this.db.prepare('SELECT dep_name AS name, dep_version AS version, dep_type AS kind FROM dependencies WHERE package_name = ? ORDER BY dep_type, dep_name').all(packageName);
        return rows;
    }
    /** Every plugin that depends on `depName` (reverse edges for impact analysis). */
    dependentsOf(depName) {
    ...
    // ---- ratings / reviews ---------------------------------------------------
    upsertRating(packageName, actor, score, comment) {
    ...
    listReviews(packageName) {
        const rows = this.db.prepare('SELECT * FROM ratings WHERE package_name = ? ORDER BY updated_at DESC, created_at DESC').all(packageName);
        return rows.map((row) => ({
            id: Number(row.id ?? 0),
            packageName: String(row.package_name ?? ''),
    ...
    ratingStats(packageName) {
        const row = this.db.prepare('SELECT AVG(score) AS avg, COUNT(*) AS count FROM ratings WHERE package_name = ?').get(packageName);
    ...
    versionCounts() {
        const map = new Map();
        const rows = this.db.prepare('SELECT package_name, COUNT(*) AS c FROM versions GROUP BY package_name').all();
        for (const r of rows)
            map.set(r.package_name, Number(r.c));
        return map;
    }
    /** Most recent mirror sync time (any plugin row), or null. */
    lastSyncedAt() {
        const row = this.db.prepare('SELECT MAX(synced_at) AS at FROM plugins').get();
        return row?.at ?? null;
    }
    /** Update the trust level for a package (from the ecosystem index). */
    setTrustLevel(packageName, level) {
    ...
    catalogRepos() {
        const set = new Set();
        const rows = this.db.prepare('SELECT repo_full_name FROM catalog_plugins').all();
        for (const r of rows)
            set.add(r.repo_full_name);
        return set;
    }
    ...
    queryAudit(query = {}) {
        ...
        const total = Number(this.db.prepare(`SELECT COUNT(*) AS n FROM audit_log${clause}`).get(...params).n);
        const limit = Math.min(Math.max(query.limit ?? 100, 1), 500);
        const offset = Math.max(query.offset ?? 0, 0);
        const rows = this.db.prepare(`SELECT id, actor, action, target, result, message, created_at AS createdAt FROM audit_log${clause} ORDER BY id DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
        return { total, rows };
    }
}
/** Convenience: instantiate once per host plugin and reuse. */
let shared;
export function storeDB(home = resolveDshHome()) {
