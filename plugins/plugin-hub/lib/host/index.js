...
import { configureDesktopPnpm, validPackageName, resolveProfile, installPlugin as runInstall, uninstallPlugin as runUninstall, setPluginEnabled, gitSpecFor, } from './installer.js';
...
let PluginStoreService = (() => {
    ...
    return class PluginStoreService extends _classSuper {
        ...
        static inject = ['loader', 'desktopPnpm'];
        ...
        constructor(ctx, config = {}) {
            ...
            configureDesktopPnpm(ctx.desktopPnpm);
            void config;
            this.scheduleTimer();
        ...
        async installPlugin(packageName, actorRaw, sourceRaw) {
            ...
            if (!validPackageName(name)) {
                this.appendAudit('install', name.slice(0, 120), 'failed', 'invalid package name', actor);
                return { ok: false, message: '插件包名格式无效', detail: null, restartRequired: false, reloadRequired: false, packageName: name };
            }
            if (name === SELF) {
        ...
        async installed() {
            ...
            for (const [name, spec] of Object.entries(deps)) {
                ...
                try {
                    ...
                    const labelMap = {
                        core: '核心服务', ui: '界面增强', tool: '开发工具', llm: '模型与 LLM', agent: 'Agent', workflow: '工作流', data: '数据处理', notification: '通知', other: '其他插件',
                    };
                    group = typeof declared === 'string' && declared ? declared : isDsh ? 'tool' : 'other';
