...
> 未完成事项已重规划（2026-08-20）：见未来计划第 5 节“未完成事项总清单与重规划”。
...
- 体验修正：多 AI 发送后，用户消息与主代理最终报告（或失败原因）以消息卡直接写入对话流（`wb-chat-flow`），
  观感与正常对话一致；底部浮层只保留“子代理运行情况”（主/子代理状态，正在工作的代理高亮）。浮层行为：
  任务进行时自动展开、可手动折叠（手动状态优先，新任务自动重置）、完成后自动折叠为标题条。模式栏移到
  头部下方右侧，不再遮挡右上角会话用量。消息按会话持久化到 localStorage（`wb.chatFlow.<sessionId>`）。

## 二点五、P2.7（2026-08-20，已部署并通过 GUI 验收）

- 单 AI / 多 AI 切换移到左侧栏“临时想法”输入框下方，按会话记忆；对话区模式栏移除。
- 多 AI 协作任务同步进入任务看板（带“AI 协作”标识）与复盘：创建编排即生成任务卡，阶段自动映射
  （规划/执行/验收→进行中，已验收→已完成，失败/取消→阻塞）；卡片点击跳转编排详情；拖拽到已完成=
  验收通过、到阻塞=确认后取消（安全回写）。
- 右侧工具栏新增“项目配置”栏：会话专属内容（存会话目录 `dsh-workbench-session.md`）、项目规则
  （发现 AGENTS.md/CLAUDE.md/README 等，可追加/初始化 AGENTS.md）、项目备注（AI 自动精炼 + 表单）。
- 新增 Host 接口：`session-context`、`project-rules`、`project-context/refine`；项目规则自动注入
  多 AI 规划提示词。
- 验证：ESLint 零报错、8 项 smoke（collab 新增 P2.7 断言）、真实 GUI 回归（左侧栏切换、看板协作卡、
  复盘、项目配置面板、失败路径展示）；部署备份 `backups/workbench-p27-20260820`。

### 排后项收尾（2026-08-20）

- 窄窗口 980px 布局实测通过（无横向溢出）。
- 规划 JSON 修复：截断补全 + 失败自动重试一次 + 输出上限提升。
- “始终编排”多代理真实执行：方案生成 3 子代理、模型分配、watchdog 重试、状态同步、失败展示均正常；
  worker1 重试后完成，worker2/3 重试后仍失败。已知限制：复杂子代理任务执行超时/子代理会话偶发报错；
  worker 超时默认 5 分钟→15 分钟，子代理会话稳定性列入专项待办（`tools/gui-p27-regression.mjs`）。
