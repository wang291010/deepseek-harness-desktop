...
- 子代理池：`agents/list|write` 接口 + JSON 编辑器（默认 7 类专家），拆解提示词携带能力匹配。
...
2. P2.5 多AI协作工作台：2.5-A/B 已完成（见上），下一步 2.5-C（记忆快照 + Token 恢复、项目上下文
   注入、成本护栏）——开工前确认 D16（快照范围）；随后 P3 监控页 → P4 工作流页 → P5 知识库与蒸馏
   → P6 打磨文档。
