...
function macWelcome(
  ...
  return [
    ...
    "printf '\\033[2J\\033[3J\\033[H'",
      `printf '%s\\n' ${quoteSh(`DeepSeek Harness Desktop ${options.productVersion} terminal`)}`,
    ...
    `printf '  %s\\n' ${quoteSh(pluginUpdate)}`,
      `printf '%s\\n' ${quoteSh('Restart DeepSeek Harness Desktop after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("DeepSeek Harness Desktop {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart DeepSeek Harness Desktop after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(DeepSeek Harness Desktop !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart DeepSeek Harness Desktop after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "DeepSeek Harness Desktop" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
