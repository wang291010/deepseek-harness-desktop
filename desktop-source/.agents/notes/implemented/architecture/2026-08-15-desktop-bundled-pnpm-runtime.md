...
The launcher captures the ordinary layered launch-environment snapshot first. It then generates a private runtime-command directory below Electron user data and prepends only its public `pnpm` command directory to the Electron main process `PATH` before profile preparation and Cordis boot. The immutable launch snapshot therefore retains the real inherited provenance, while direct third-party libraries and the DSH subprocess provider inherit the runtime command through `process.env`.
...
**Put the path into the launch-environment snapshot.** The snapshot records where user-visible launch values came from; a generated product command is not an inherited, project, or user `.env` value.
