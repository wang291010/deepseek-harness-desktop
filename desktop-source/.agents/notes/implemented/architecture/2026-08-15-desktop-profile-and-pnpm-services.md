...
`runPlugin(args, invokingDir, signal?)` starts the packaged `dsh plugin --profile <active>` command with `invokingDir` as the CLI working directory. The arguments after the profile are pnpm arguments such as `add`, `remove`, `update`, or recovery-time `install --no-frozen-lockfile`. The upstream CLI remains authoritative for anchoring relative `file:` and `link:` specifications, changing into the profile directory for pnpm, initializing a missing profile when applicable, and reconciling `dsh.profile.bundles` after a successful mutation.

Plugin managers must therefore use `runPlugin()`. They own their user-facing deadline and progress model, consume the streaming output, abort on timeout, and wait for `done`; the service owns executable selection, child environment, the single-operation gate, process-tree termination, and the active-profile selection with no consumer-supplied profile argument.

## Third-party compatibility and dshmarket

An optional Desktop-aware consumer resolves `desktopProfiles` and `desktopPnpm` dynamically after its ordinary required Host services are available. When both exist, the immutable service identity overrides config or argv guesses and plugin mutations use `runPlugin()`. When neither exists, the consumer keeps its existing config/argv and DSH CLI path so the same package continues to work in an ordinary Web profile. A Desktop service must not become a required Cordis injection for a cross-environment plugin.
